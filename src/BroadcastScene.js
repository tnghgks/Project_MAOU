...
const MONSTERS = {
  slime: { name: '슬라임', mp: 10, hp: 20, dmg: 3, atkCd: 1.0, speed: 60, range: 24, gold: 5, color: 0x55cc44, size: 16 },
  archer: { name: '고블린 궁수', mp: 20, hp: 15, dmg: 4, atkCd: 2.0, speed: 70, range: 240, gold: 8, color: 0xddaa33, size: 14 },
  golem: { name: '골렘', mp: 35, hp: 120, dmg: 8, atkCd: 1.5, speed: 35, range: 30, gold: 20, color: 0x8899aa, size: 28 },
};
const KEYS = ['D', 'F', 'J', 'K'];
...
export default class BroadcastScene extends Phaser.Scene {
  ...
  create() {
    // ── 상태 ──
    this.hero = { x: 470, y: 300, hp: 100, maxHp: 100, atk: 10, atkSpd: 1.0, speed: 100, range: 60, atkCd: 0, retreatT: 0, retreatCd: 0 };
    this.monsters = [];
    this.arrows = [];
    this.mp = 100;
    this.viewers = 12;
    ...
    this.selectedType = 'slime';
    this.summonCd = { slime: 0, archer: 0, golem: 0 };
    ...
    this.input.keyboard.on('keydown', (e) => {
      ...
      if (k === '1') this.selectType('slime');
      else if (k === '2') this.selectType('archer');
      else if (k === '3') this.selectType('golem');
      else if (KEYS.includes(k)) this.hitNote(k);
    });

    this.pushChat('시스템', '방송이 시작되었습니다.', '#888888');
  }
  ...
  buildUI() {
    const add = this.add;
    // 배경 패널
    add.rectangle(640, 20, 1280, 40, 0x1a1a24).setDepth(5); // 상단바
    add.rectangle(CHAT_X + 170, 380, 340, 680, 0x14141c).setDepth(5); // 채팅
    add.rectangle(470, (SUMMON_Y + LANE_Y) / 2, 940, LANE_Y - SUMMON_Y, 0x1a1a24).setDepth(5); // 소환바
    add.rectangle(470, (LANE_Y + 720) / 2, 940, 720 - LANE_Y, 0x0d0d14).setDepth(5); // 리듬 레인
    ...
    this.chatLines = [];
    this.chatContainer = add.container(0, 0).setDepth(6);

    // 소환 버튼
    this.summonBtns = {};
    let bx = 20;
    for (const [k, m] of Object.entries(MONSTERS)) {
      const btn = add.rectangle(bx, SUMMON_Y + 14, 180, 30, 0x2a2a3a).setOrigin(0).setDepth(6).setInteractive();
      const label = add.text(bx + 8, SUMMON_Y + 20, `${m.name}  MP${m.mp}`, { fontSize: '15px', color: '#ffffff' }).setDepth(7);
      btn.on('pointerdown', (p, lx, ly, ev) => { ev.stopPropagation(); this.selectType(k); });
      this.summonBtns[k] = btn;
      bx += 195;
    }
    add.text(bx + 10, SUMMON_Y + 20, '(1/2/3 선택 → 아레나 클릭)', { fontSize: '13px', color: '#666677' }).setDepth(6);
  ...
  // ── 스킬 (낙뢰) ──
  // ponytail: 티어0은 스킬 1종 고정. MVP에서 4종+랜덤 선택으로 확장
  fireSkill(res) {
    if (res.penalty) {
    ...
    const dmg = 30 * res.mult;
    const radius = res.clear ? Infinity : 260;
    for (const m of this.monsters) {
      if (Phaser.Math.Distance.Between(m.x, m.y, this.hero.x, this.hero.y) <= radius) {
    ...
    this.floatText(this.hero.x, this.hero.y - 40, `⚡ ${res.grade} ×${res.mult}`, '#ffee44');
    if (res.clear) for (const line of CHAT_POOLS.allperfect) this.pushChat('시청자', line, '#ffee44');
  ...
  damageMonster(m, dmg) {
    m.hp -= dmg;
    m.spr.setAlpha(0.5);
    this.time.delayedCall(80, () => m.spr.setAlpha(1));
    if (m.hp <= 0 && !m.dead) {
      m.dead = true;
      this.gold += m.def.gold;
      this.kills++;
      m.spr.destroy();
    }
  }
  ...
  pushChat(who, msg, color = '#cccccc') {
    const t = this.add.text(CHAT_X + 12, 0, `${who}: ${msg}`, {
      fontSize: '14px', color, wordWrap: { width: 316 },
    }).setDepth(6);
  ...
  updateHero(dt, nearCount) {
    ...
    if (H.retreatT > 0 && alive.length) {
      ...
      // 반경 300 내 최근접 몬스터로 이동, 없으면 중앙 복귀
  ...
  updateChat(dt, D) {
    ...
    const lps = clamp(1 + (this.viewers / 5000) * 7, 1, 8); // 초당 줄 수
    this.chatT = 1 / lps;
  ...
  updateHUD(D, tier) {
    ...
    this.goldText.setText(`💰 ${Math.floor(this.gold).toLocaleString()}G`);
